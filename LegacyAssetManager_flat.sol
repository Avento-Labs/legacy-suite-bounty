...
interface ILegacyVaultFactory {
    function createVault(address _memberAddress, address[] calldata _wallets)
        external;
    ...
    function addWallet(address[] calldata _wallets) external;
...
contract LegacyAssetManager is AccessControl, Pausable, ReentrancyGuard {
    ...
    event UserVaultCreated(string userId, address owner, address[] wallets);
    ...
    function createUserVault(
        ...
        vaultFactory.createVault(_msgSender(), _wallets);
        emit UserVaultCreated(userId, _msgSender(), _wallets);
    }
    ...
    function removeERC20Single(string memory userId, address _contract)
        external
        nonReentrant
    {
